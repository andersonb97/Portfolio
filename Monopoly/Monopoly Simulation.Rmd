...
jail <- function(i){
  if (tail(doubles[[i]], 1) == 1) {
    ...
    rent()
             ...
             || sum(spots[[i]][(length(spots[[i]]) - 2) : (length(spots[[i]]))] == 100) == 3) {
...
monopoly <- function() {
  ...
  while(sum(bankrupt.players > 0) < 5) {
  ...
  results.df <<- data.frame(players, player.cash, player.comfort, bankrupt.players, turn, 
                           total.value.fifteen, owned.blue, times.in.jail, properties.sold, color.sets, props.at.five)
...
This code runs a Monte Carlo simulation study and compiles all results into two dataframes and writes them as '.csv' documents. It is commented out so that you can run the full R-Markdown file without the 3 hour long simulation.
```{r}
# tic()
# n.reps <- 3000
# monte.carlo <- replicate(n.reps, monopoly())
# 
